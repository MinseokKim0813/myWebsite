import { RESEARCH_LINKS, type SiteContent } from "./shared";

const skillItems = {
  languages: [
    "Python",
    "C++",
    "C",
    "Java",
    "JavaScript",
    "Assembly",
    "SQL",
    "HTML",
    "CSS",
    "YAML",
    "RegEx",
  ],
  frameworks: [
    "PyTorch",
    "React",
    "Node.js",
    "Express",
    "Spring",
    "Flask",
    "FastAPI",
    "pytest",
    "Selenium",
    "NumPy",
    "Pandas",
    "asyncio",
    "REST API",
    "Prometheus",
    "Grafana",
    "Docker",
    "Git",
    "GitHub",
    "GitLab",
    "MATLAB",
    "Figma",
    "FlutterFlow",
    "Wireshark",
  ],
  databases: ["AWS RDS", "Firebase", "MongoDB", "HPC", "ArgoCD"],
  ai: ["LLM", "컴퓨터 비전", "자연어 처리", "메커니스틱 해석"],
};

export const koContent: SiteContent = {
  hero: {
    nameFull: "김민석",
    nameGiven: "김민석",
  },
  about: {
    bioComment: "/* 약력 */",
    bioParagraphs: [
      "2026년 5월 NYU Abu Dhabi에서 컴퓨터공학 학사 및 응용수학 부전공(GPA: 3.7/4.0)으로 졸업했습니다. 확장 가능한 시스템 구축과 데이터 및 AI를 실용적인 소프트웨어 솔루션에 녹여내는 일에 열정을 가지고 있으며, 데이터 엔지니어링, 백엔드 개발, 머신러닝에 이르는 다양한 영역에서 경험을 쌓아왔습니다.",
      "가장 최근에는 NYUAD eBRAIN Lab의 연구원(Research Assistant)으로서 HPC 클러스터 기반 오디오 언어 모델의 보안 및 취약점 연구, 그리고 탈옥(Jailbreak) 방어를 위한 매커니즘 해석가능성(Mechanistic Interpretability) 연구를 진행했습니다. 또한, LLM을 활용해 문서를 무제한의 대화형 게임식 퀴즈로 변환해 주는 iOS 애플리케이션 'Brainlot'의 공동 창업자이기도 합니다. Brainlot은 1,000회 이상의 다운로드를 기록하며 NYUAD 스타트업 인큐베이터에 선정되었습니다.",
      "실무에서는 뉴욕 XBTO Trading의 소프트웨어 엔지니어(Prometheus 및 Grafana를 활용한 실시간 성능 모니터링, 가상자산 시장을 위한 Oracle 데이터 파이프라인 엔지니어링), 두바이 Swop Holdings의 풀스택 개발자(핵심 인프라의 Firebase→AWS 이전 및 프리시드 단계의 제품 비전 기여), Liwa Capital의 소프트웨어 엔지니어(LLM 기반 이메일 처리 및 Microsoft API 활용 맞춤형 데이터 파이프라인 구축), 한국 IT BAND의 백엔드 개발자(Java Spring을 활용한 한국해양교통안전공단 웹사이트 리뉴얼)로 참여하며 역량을 다졌습니다.",
      "실무 외에도 실시간 인기 맛집 찾기 플랫폼 'Foodify', 영화 포스터 기반 장르 분류 ML 모델 'DeepGenre', 90% 이상의 테스트 커버리지를 갖춘 커서 감도 최적화 데스크톱 툴 'SenseFit' 등 다양한 사이드 프로젝트를 지속적으로 개발해 왔습니다. 복잡한 문제를 코드 연구로 해결하고 새로운 기술을 습득하는 과정 자체를 언제나 즐깁니다.",
    ],
    educationTitle: "학력",
    educationSchool: "뉴욕대학교 아부다비 (NYU Abu Dhabi)",
    educationDegree: "이학사 (B.S.) · 컴퓨터공학 전공 · 응용수학 부전공",
    educationDetails:
      "2020년 – 2026년 · 2026년 5월 졸업 · 학점: 3.7 / 4.0 · 아랍에미리트 아부다비",
    educationCoursework:
      "주요 이수 과목: 머신러닝, 자료구조, 알고리즘, 컴퓨터 비전, 컴퓨터 네트워크, 소프트웨어 공학, 운영체제 등",
    statYearsCoding: "코딩 경력",
    statProjects: "프로젝트",
    statResearch: "연구",
    skills: [
      { category: "프로그래밍 언어", items: skillItems.languages },
      { category: "프레임워크 & 도구", items: skillItems.frameworks },
      { category: "데이터베이스 & 인프라", items: skillItems.databases },
      { category: "AI/ML", items: skillItems.ai },
    ],
  },
  projects: {
    featuredLabel: "주요 프로젝트",
    items: [
      {
        id: "brainlot",
        title: "Brainlot",
        subtitle: "무한 스크롤 기반 학습 플랫폼",
        tech: ["백엔드", "모바일 앱", "데이터베이스"],
        highlights: [
          "사용자가 업로드한 문서를 인터랙티브한 게임화된 객관식 퀴즈의 연속으로 자동 변환하는 스크롤 인터페이스 기반 iOS 애플리케이션을 공동 창업 및 출시하여 1,000건 이상의 다운로드를 기록함.",
          "끊김 없는 스크롤을 지원하기 위해 백그라운드에서 무한 퀴즈 생성 파이프라인을 구축했으며, 사용자가 클라우드 기반 모델과 로컬 LLM 간 전환이 가능한 유연한 추론 아키텍처를 설계함.",
          "NYUAD 스타트업 인큐베이터에 선정되어 전문적인 지도를 통해 제품 성장을 가속화하고, 시장에서의 잠재력과 독창성을 검증받음.",
        ],
        external:
          "https://apps.apple.com/kr/app/brainlot/id6754814263?l=en-GB",
        image: "gradient-1",
        imageSrc: "brainlot-preview.png",
      },
      {
        id: "foodify",
        title: "Foodify",
        subtitle: "트렌디한 맛집 탐색 소셜 플랫폼",
        tech: [
          "백엔드",
          "웹 앱",
          "JavaScript (React)",
          "Python",
          "웹 스크래핑",
          "데이터베이스",
        ],
        highlights: [
          "사용자가 스와이프 제스처로 트렌딩 맛집을 발견하고, 리스트를 공유하며 인기 추천을 탐색할 수 있는 소셜 플랫폼을 구축함.",
          "소셜 미디어와 유사한 경험을 통해 사용자들이 취향을 공유하며 쉽게 연결되도록 돕고, 소상공인들이 대중에게 효과적으로 노출될 수 있도록 지원함.",
          "옐프(Yelp)와 구글 맵(Google Maps) 데이터를 스크래핑하고, 파이어베이스(Firebase)를 활용하여 데이터를 관리함.",
        ],
        external: "https://foodify.cc",
        image: "gradient-2",
        imageSrc: "foodify-preview.png",
      },
      {
        id: "sensefit",
        title: "SenseFit",
        subtitle: "커서 감도 자동 최적화 툴",
        tech: ["Python", "PyQt5", "pytest"],
        highlights: [
          "실시간으로 마우스 커서 데이터를 추적하고 움직임 패턴을 분석하여, 사용자에게 최적의 마우스 감도(DPI)를 계산하고 제안하는 데스크톱 애플리케이션을 개발함.",
          "분석 알고리즘을 설계하여 비효율적인 커서 움직임과 같은 최적화되지 않은 조작 습관을 식별하고, 이를 바탕으로 데이터 기반의 개선안을 제공함.",
          "pytest를 활용해 90% 이상의 단위(Unit) 및 시스템 테스트 커버리지를 달성함으로써 소프트웨어의 높은 신뢰성을 확보함.",
        ],
        github: "https://github.com/MinseokKim0813/Sense_Fit",
        external: "https://github.com/MinseokKim0813/Sense_Fit",
        image: "gradient-3",
        imageSrc: "sensefit-preview.png",
      },
      {
        id: "deepgenre",
        title: "DeepGenre",
        subtitle: "딥러닝 기반 영화 장르 분류 모델",
        tech: ["머신러닝", "컴퓨터 비전", "PyTorch", "Python"],
        highlights: [
          "영화 포스터 이미지를 기반으로 28개의 고유한 장르를 예측하는 멀티 레이블(Multi-label) CNN 모델을 개발함.",
          "59.98%의 정밀도(Precision)를 달성하여 무작위 예측(Random Baseline) 대비 16.8배 높은 성능을 기록했으며, 복잡한 시각 인식 작업에서 강력한 전이 학습(Transfer Learning) 성능을 입증함.",
          "OCR 및 인페인팅(In-painting) 기술을 활용해 포스터 내 텍스트를 제거함으로써, 기존 모델들에서 발생하던 제목 텍스트 기반의 예측 편향을 없애고 모델이 오직 시각적 요소(Artwork)에만 의존하여 분류하도록 설계함.",
        ],
        github: "https://github.com/MinseokKim0813/DeepGenre",
        external: "https://github.com/MinseokKim0813/DeepGenre",
        image: "gradient-4",
        imageSrc: "deepgenre-preview.png",
      },
      {
        id: "genderar",
        title: "GenderAr",
        subtitle: "아랍어 이름 성별 분류 모델 (연구 보고서)",
        tech: ["머신러닝", "자연어 처리(NLP)", "트랜스포머(Transformers)", "Python"],
        highlights: [
          "특정 언어적 규칙을 활용하는 형태소 분석기(Morphological Analyzer)와 딥러닝 추론을 위해 파인튜닝된 트랜스포머(Transformer) 등 두 가지 맞춤형 학습 모델을 결합하여, 아랍어 이름에 최적화된 새로운 하이브리드 성별 분류기 개발.",
          "언어적 규칙과 딥러닝 추론을 성공적으로 통합함으로써 베이스라인 모델의 성능을 뛰어넘는 91.68%의 정확도(Accuracy) 달성.",
          "형태소 분석 모델(로지스틱 회귀), 의미 분석 모델(워드 임베딩), 딥러닝 모델(파인튜닝 트랜스포머)의 3가지 개별 분류 방법론과 하이브리드 캐스케이드(Hybrid Cascade) 시스템을 구현하고 성능을 비교 평가.",
        ],
        github: "https://github.com/MinseokKim0813/GenderAr",
        external: RESEARCH_LINKS.gender,
        image: "gradient-5",
        imageSrc: "genderar-preview.png",
      },
      {
        id: "mathtextor",
        title: "MathTextor",
        subtitle: "수식 조판의 인간화 (연구 보고서)",
        tech: ["풀스택", "웹 앱", "인간-컴퓨터 상호작용(HCI)", "JavaScript"],
        highlights: [
          "문제 문맥을 분석해 수학 기호를 동적으로 예측하고 제안하는 LLM 기반의 문맥 인식(Context-aware) 웹 인터페이스를 개발하여, 사용자가 복잡한 LaTeX 문법을 암기해야 하는 불편함을 해소.",
          "24명의 참가자를 대상으로 사용자 연구(User study)를 진행하여, 초보 사용자가 수식을 작성하는 데 걸리는 시간을 기존 LaTeX 에디터 대비 38.6% 단축했음을 입증.",
          "72.9%의 F1 점수(F1 Score)를 기록한 LLM 추론 파이프라인을 구축하고 HCI(인간-컴퓨터 상호작용) 디자인 원칙을 적용해 사용자 경험을 최적화했으며, 결과적으로 7점 만점에 4.57점의 사용성 점수(Usability score)를 달성.",
        ],
        github: "https://github.com/MinseokKim0813/MathTextor",
        external: RESEARCH_LINKS.mathTextor,
        image: "gradient-6",
        imageSrc: "mathtextor-preview.png",
      },
      {
        id: "trendmap",
        title: "TrendMap",
        subtitle: "실시간 미디어 관심도 분석 툴",
        tech: ["데이터 분석", "웹 스크래핑", "Python"],
        highlights: [
          "미국 주요 기업들의 언론 언급량(Media mentions)을 추적하고, 산업 분야 및 주(State)별 트렌드를 다각도로 분석하는 툴 개발.",
          "가장 높은 미디어의 관심을 받는 산업군(Sector)과 지역을 식별하여, 데이터 기반의 투자 결정 및 시장 분석을 위한 유의미한 인사이트 제공.",
        ],
        github: "https://github.com/MinseokKim0813/TrendMap",
        external: "https://github.com/MinseokKim0813/TrendMap",
        image: "gradient-7",
        imageSrc: "trendmap-preview.png",
      },
      {
        id: "fundmonitor",
        title: "FundMonitor",
        subtitle: "크라우드펀딩 캠페인 데이터 스크래퍼 및 분석 툴",
        tech: ["데이터 분석", "웹 스크래핑", "Python"],
        highlights: [
          "크라우드펀딩 플랫폼에서 실시간 데이터를 추출하고 분석하는 웹 스크래퍼를 개발하여, 다양한 카테고리 및 지역별 펀딩 진행 현황에 대한 심층적인 인사이트 제공.",
          "글로벌 펀딩 트렌드와 자금 조달이 부족한 섹터(Underfunded sectors)를 식별함으로써, 최적화된 기업의 사회적 책임(CSR) 전략 수립, 타겟 투자, 그리고 제품 혁신 기회 발굴을 지원.",
        ],
        github: "https://github.com/MinseokKim0813/FundMonitor",
        external: "https://github.com/MinseokKim0813/FundMonitor",
        image: "gradient-8",
        imageSrc: "fundmonitor-preview.png",
      },
    ],
  },
  experiences: [
    {
      company: "eBRAIN Lab",
      role: "연구원 (Research Assistant)",
      period: "2026/01 - 2026/07",
      location: "아부다비, 아랍에미리트",
      description: [
        "고성능 컴퓨팅(HPC) 클러스터를 활용하여 대규모 오디오 언어 모델(ALM)의 보안 메커니즘을 심층 분석하고 관련 취약점을 연구함.",
        "모델의 내부 벡터를 추적, 이해하고 메커니즘적 해석가능성 방법론을 적용하여 유해 답변을 유발하는 구체적인 보안 취약점 요인을 규명.",
      ],
    },
    {
      company: "XBTO Trading",
      role: "소프트웨어 엔지니어링 인턴",
      period: "2025/06 - 2025/08",
      location: "뉴욕, 미국",
      description: [
        "Prometheus와 Grafana를 활용하여 경쟁사 순위 및 개별 암호화폐 점수를 시각화하는 실시간 성능 모니터링 시스템을 설계 및 배포함.",
        "Oracle 데이터 엔지니어링 팀의 일원으로서 외부 구매 미가공 시장 데이터를 수집하는 핵심 데이터 파이프라인을 유지보수 및 디버깅하고, 데이터 무결성과 시스템 안정성을 보장하기 위해 개별 처리 작업의 배포를 관리함.",
        "웹 스크래핑을 통해 경쟁사 및 자산 성과 데이터를 수집·분석하여, 암호화폐 퍼블리싱 분야에서 외부 기업 순위를 높이기 위한 핵심 데이터를 제공함.",
      ],
    },
    {
      company: "Swop Holdings",
      role: "풀스택 개발자",
      period: "2025/02 - 2025/05",
      location: "두바이, 아랍에미리트",
      description: [
        "프리시드(Pre-seed) 단계의 초기 멤버로 합류하여 제품 비전 수립 및 벤처 캐피털(VC) 투자 유치에 직접 기여함.",
        "데이터베이스 설계, 데이터 이관 및 백엔드 엔드포인트 리팩토링을 수행하며, 핵심 데이터 인프라를 Firebase에서 AWS RDS로 마이그레이션함.",
        "데이터 운영을 관리하며 파트너사의 제품 데이터를 정제 및 구조화하고, 관계형 데이터베이스로의 정확한 데이터 적재를 보장함.",
      ],
    },
    {
      company: "Liwa Capital",
      role: "소프트웨어 엔지니어링 인턴",
      period: "2024/04 - 2024/07",
      location: "아부다비, 아랍에미리트",
      description: [
        "LLM을 시스템에 통합하여 이메일 읽기, 일정 동기화, 그룹 업무 할당 및 실행 가능한 인사이트 도출을 수행하는 맞춤형 자동화 소프트웨어를 개발해 팀에 도입함.",
        "LLM API, Microsoft API 및 사내 문서 데이터베이스를 연동하여 보안이 보장된 맞춤형 데이터 처리 파이프라인을 설계 및 구축함.",
        "일일 수백 통의 이메일을 수동으로 확인하는 프로세스를 전면 자동화하여 팀의 전반적인 업무 생산성을 대폭 향상함.",
      ],
    },
    {
      company: "아이티밴드",
      role: "백엔드 개발자",
      period: "2021/08 - 2021/10",
      location: "대전, 대한민국",
      description: [
        "정부 안전·교통 관리 기관의 대규모 정보 플랫폼 개편 프로젝트를 수행하여 시스템 성능을 최적화하고 사용자 경험(UX)을 현대화함.",
        "백엔드 최적화 및 디버깅에 주력하여 로딩 시간을 단축하고 치명적인 시스템 오류를 해결함으로써 성능과 안정성을 개선함.",
        "Java Spring Framework, Oracle, HTML, JavaScript를 활용하여 백엔드 시스템을 고도화함.",
      ],
    },
  ],
  research: {
    interests: [
      {
        title: "머신러닝",
        description: "딥러닝, 신경망 구조 탐색, 모델 경량화",
      },
      {
        title: "자연어 처리",
        description: "Transformer, 다국어 모델, 의미 이해",
      },
      {
        title: "HCI",
        description: "사용자 중심 설계, 사용성 평가, 인터랙티브 시스템",
      },
      {
        title: "LLM 보안",
        description: "프롬프트 인젝션, 모델 견고성, 안전한 LLM 배포",
      },
    ],
    current: {
      role: "연구 보조",
      lab: "eBRAIN Lab",
      institution: "NYU Abu Dhabi",
      period: "2026년 1월 – 7월",
      roleMeta: "NYU Abu Dhabi 연구 보조 · 2026년 1월 – 7월",
      topic: "LLM 탈옥 보안",
      summary:
        "오디오에 숨겨진 유해 지시가 음성 AI 모델을 속이는 방식과, 그 공격이 모델 내부 어디에서 작동하는지를 연구합니다. 탈옥 신호는 중·고역 mel 대역과 아키텍처별 디코더 깊이에 집중되며, 모델 간 공격 전이는 실패하지만 대역별 비대칭을 드러내어 대역 억제와 레이어 모니터링 기반 방어 방향을 제시합니다.",
      authors: [
        "Boyuan Chen [NYU]",
        "Sohaila Abdulsattar [NYU]",
        "김민석 [NYU]",
        "Minghao Shao [NYU]",
        "Siddharth Garg [NYU]",
        "Ramesh Karri [NYU]",
        "Muhammad Shafique [NYU]",
      ],
      link: RESEARCH_LINKS.ebrain,
    },
    publications: [
      {
        title:
          "아랍어 이름의 성별 판별: 형태·의미·딥러닝 접근 비교 연구",
        type: "연구 논문",
        authors: ["김민석 [NYU Abu Dhabi]"],
        advisors: "Nizar Habash [NYU Abu Dhabi]",
        abstract:
          "형태소 분석기·언어 규칙·파인튜닝 Transformer를 결합한 아랍어 이름 성별 분류 하이브리드 모델을 제안했습니다. 정확도 91.68%로, 규칙과 딥러닝을 함께 쓰는 방식이 단독 모델보다 우수함을 보였습니다.",
        link: RESEARCH_LINKS.gender,
      },
      {
        title: "MathTextor: 학부생을 위한 수식 조판 UX 개선",
        type: "연구 논문",
        authors: ["김민석 [NYU Abu Dhabi], Moumena Chaqfeh [NYU Abu Dhabi]"],
        advisors:
          "Moumena Chaqfeh [NYU Abu Dhabi], Steven Euijong Whang [KAIST]",
        abstract:
          "문제 맥락에 맞는 수학 기호를 LLM이 제안하는 맥락 인식 웹 인터페이스입니다. 24명 사용자 연구에서 초보자 조판 시간 38.6% 단축, F1 72.9%, 사용성 4.57/7을 달성했습니다.",
        link: RESEARCH_LINKS.mathTextor,
      },
    ],
    advisedBy: "지도 교수:",
  },
};
